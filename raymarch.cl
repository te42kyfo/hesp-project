...
__kernel void raymarch( global real* density_field,
  ...
  //AABB advance
  float4 txyz0 = min( max( 0.0f, (lo_bound + 1.0f*h - origin) / dir),
		      max( 0.0f, (hi_bound - 1.0f*h - origin) / dir) );
  ...
  while( all( (current_index >= 0.0f && current_index < (fcell_count-1.0f)).xyz) ) {
	...
	float3 normal = calculateNormal( density_field,
